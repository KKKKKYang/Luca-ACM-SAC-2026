...
class GATedge(nn.Module):
    ...
    '''
    def __init__(self,
                 in_feats,#(self.in_size_ope, self.in_size_ma) 
        ...
        #self.in_size_ope = model_paras["in_size_ope"]  # Dimension of the raw feature vectors of operation nodes
                 out_feats,#self.out_size_ma
        #self.out_size_ma = model_paras["out_size_ma"]  # Dimension of the embedding of machine nodes
                 num_head,#self.num_head
                 feat_drop=0.,#self.dropout
                 attn_drop=0.,#self.dropout
                 negative_slope=0.2,#
                 residual=False,
                 activation=None):#activation=F.elu
        '''
        ...
        # print('in_feats[0]:', in_feats[0])
        # print('in_feats[1]:', in_feats[1])
        # print('out_feats:', out_feats)

        self._in_src_feats = in_feats[0] #self.in_size_ope
        self._in_dst_feats = in_feats[1] #self.in_size_ma
        # print("in_src_feats",self._in_src_feats,"in_dsc_feats", self._in_dst_feats)
    ...
    def forward(self, ope_ma_adj_batch, batch_idxes, feat):
        ...
        # grab the adjacency slice (already on the right device)
        x = ope_ma_adj_batch[batch_idxes]   # shape [B, ops, M]
        device = x.device

        # mask real edges
        mask_edges = (x.unsqueeze(-1) == 1)  # → [B, ops, M, 1]

        # build the all-True diagonal block on the same device
        mask_diag = torch.full(
            (x.size(0), 1, x.size(2), 1),     # [B, 1, M, 1]
            True,
            dtype=torch.bool,
            device=device                     # ← **here** we force CUDA
        )

        # concatenate
        mask = torch.cat((mask_edges, mask_diag), dim=-3)
