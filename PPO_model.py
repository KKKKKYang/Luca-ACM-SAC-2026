...
class HGNNScheduler(nn.Module):
    def __init__(self, model_paras):
        ...
        self.check = model_paras["actor_in_dim"]
        self.actor_dim = 2*model_paras["out_size_ma"] + 2*model_paras["out_size_ope"]

        print(self.check , self.actor_dim)
    ...
    def evaluate(self, ope_ma_adj, ope_pre_adj, ope_sub_adj, raw_opes, raw_mas, proc_time,
        ...
        B, G, D_ope = h_jobs.shape
        M = raw_mas.size(1)       # same as h_mas.size(1)
        D_mas = h_mas.size(-1)

        # 1) expand h_jobs to [B, G, M, D_ope]
        h_jobs_padding = (
            h_jobs
            .unsqueeze(-2)        # [B, G, 1, D_ope]
            .expand(-1, -1, M, -1)
        )

        # 2) expand h_mas to [B, G, M, D_mas]
        h_mas_padding = (
            h_mas
            .unsqueeze(1)         # [B, 1, M, D_mas]
            .expand(-1, G, -1, -1)
        )

        # 3) expand pooled ops to [B, G, M, D_ope]
        h_opes_pooled_padding = (
            h_opes_pooled
            .unsqueeze(1).unsqueeze(2)  # [B, 1, 1, D_ope]
            .expand(-1, G, M, -1)
        )

        # 4) expand pooled mas to [B, G, M, D_mas]
        h_mas_pooled_padding = (
            h_mas_pooled
            .unsqueeze(1).unsqueeze(2)  # [B, 1, 1, D_mas]
            .expand(-1, G, M, -1)
        )

        # 5) concatenate & transpose → [B, G*M, total_dim]
        h_actions = torch.cat([
            h_jobs_padding,
            h_mas_padding,
            h_opes_pooled_padding,
            h_mas_pooled_padding
        ], dim=-1).transpose(1, 2)  # (jobs dim and machine dim swap)
...
class PPO:
    def __init__(self, model_paras, train_paras, num_envs=None):
        ...
        self.lambda_penalty = train_paras["lambda_penalty"]  # lambda for weighted sum of makespan and emission
    ...
    def update(self, memory, env_paras, train_paras):
        ...
        # + makespans & emissions
        memory_makespans = torch.stack(memory.makespans, dim=0).transpose(0,1)
        memory_emissions = torch.stack(memory.emissions, dim=0).transpose(0,1)
        ...
        # Estimate and normalize the makespans and emissions
        rewards_envs = []
        total_discounted_makespan = 0
        total_discounted_emission = 0
        total_final_reward = 0
        for i in range(self.num_envs):
            makespans = []
            emissions = []
            discounted_makespan = 0
            discounted_emission = 0
            for makespan, emission, is_terminal in zip(reversed(memory_makespans[i]),
                                                      reversed(memory_emissions[i]),
                                                      reversed(memory_is_terminals[i])):
                if is_terminal:
                    total_discounted_makespan += discounted_makespan
                    total_discounted_emission += discounted_emission
                    discounted_makespan = 0
                    discounted_emission = 0
                discounted_makespan = makespan + (self.gamma * discounted_makespan)
                discounted_emission = emission + (self.gamma * discounted_emission)
                makespans.insert(0, discounted_makespan)
                emissions.insert(0, discounted_emission)
            total_discounted_makespan += discounted_makespan
            total_discounted_emission += discounted_emission

            makespans = torch.tensor(makespans, dtype=torch.float64).to(device)
            emissions = torch.tensor(emissions, dtype=torch.float64).to(device)
            makespans = (makespans - makespans.mean()) / (makespans.std() + 1e-5)
            emissions = (emissions - emissions.mean()) / (emissions.std() + 1e-5)
            #rewards = (makespans + emissions) / 2.0  # Average of makespan and emission
            rewards = (1-self.lambda_penalty)* makespans + self.lambda_penalty * emissions  # Weighted sum of makespan and emission
            rewards_envs.append(rewards)
            total_final_reward += rewards[-1].item()  # Final reward for the last step
        ...
        self.policy_old.load_state_dict(self.policy.state_dict())
        avg_final_reward = total_final_reward / self.num_envs if self.num_envs > 0 else 0

        return loss_epochs.item() / self.K_epochs, \
               avg_final_reward, \
               rewards_envs # Return the in-scope rewards_envs variable